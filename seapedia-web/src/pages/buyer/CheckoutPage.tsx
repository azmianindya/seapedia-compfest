...
function CheckoutPage() {
  ...
  const handleApplyVoucher = async () => {
    ...
    setVoucherError('')
    try {
      const res = await api.post('/vouchers/check', { code: voucherCode, subtotal })
  ...
  return (
    <PublicLayout>
      <div className="max-w-2xl mx-auto">
        ...
        {/* Voucher */}
        <div className="bg-white rounded-2xl shadow-sm p-5 mb-4">
          <div className="text-sm font-medium text-gray-700 mb-3">Voucher & Promo</div>

          {voucherApplied ? (
            <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg px-4 py-2.5">
              <div>
                <div className="text-sm font-medium text-green-700">{voucherApplied.code}</div>
                <div className="text-xs text-green-600">Hemat Rp {discount.toLocaleString('id-ID')}</div>
              </div>
              <div onClick={handleRemoveVoucher} className="text-xs text-red-500 cursor-pointer hover:underline">Hapus</div>
            </div>
          ) : (
            <div className="flex gap-2">
              <input
                type="text"
                className="flex-1 border border-gray-200 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400 uppercase"
              ...
              <div
                ...
                className={`bg-orange-500 text-white text-sm font-medium px-5 rounded-lg flex items-center cursor-pointer hover:bg-orange-600 ${checkingVoucher ? 'opacity-50 pointer-events-none' : ''}`}
              >
        ...
        <div className="bg-white rounded-2xl shadow-sm p-5">
          ...
          <div className="flex flex-col gap-2 text-sm mb-4">
            ...
            {discount > 0 && (
              <div className="flex justify-between text-green-600"><div>Diskon Voucher</div><div>- Rp {discount.toLocaleString('id-ID')}</div></div>
            )}
